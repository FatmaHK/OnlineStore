...
@CrossOrigin(origins = "*")
@RestController
public class DeleteBrand extends StoreCommand{
	...
	@GetMapping("/onlinemarket/deleteBrandFromStore/{sName}/{bName}")
	public boolean execute(@PathVariable String sName, @PathVariable String bName) {
		...
		StoreAction a = actionRepository.findByName("Delete brand");
		sc= new StoreController(storeBrandRepo, saRepository);
		return (sc.deleteBrand(s, b, a));
	}
